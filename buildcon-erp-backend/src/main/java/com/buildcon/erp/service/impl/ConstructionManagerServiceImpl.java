...
@Service
public class ConstructionManagerServiceImpl implements ConstructionManagerService {

    @Autowired
    private ConstructionManagerRepository repository;

    @Autowired
    private PasswordEncoder encoder;

    @Override
    public ConstructionManager register(GenericSignupRequest request) {
        ...
        ConstructionManager item = new ConstructionManager(
            request.getUsername(),
            request.getEmail(),
        ...
        item.setAvatarInitials(initials.isEmpty() ? "CM" : initials);
