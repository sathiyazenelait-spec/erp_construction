...
export default function AIProjectAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { sender: "ai", text: "Hello. I am your BuildWell AI Project Assistant. I have analyzed active construction milestones, PM scorecards, change orders, and material pipelines. Ask me anything about project delays, quality audits, or budget overruns.", time: "10:30 AM" }
  ]);
  ...
  useEffect(() => {
    ...
    fetch(`https://erp-construction.onrender.com/api/project-director/dashboard/org/${orgId}`, {
      ...
      .then((d) => {
        ...
        if (d.profileName) {
          setMessages([
            { sender: "ai", text: `Hello, ${d.profileName}. I am your BuildWell AI Project Assistant. I have analyzed active construction milestones, PM scorecards, change orders, and material pipelines. Ask me anything about project delays, quality audits, or budget overruns.`, time: "10:30 AM" }
          ]);
        }
      })
      .catch((err) => {
        console.error("Error fetching AI suggestions for PD:", err);
        setSuggestions([
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem("buildcon_token");
      const res = await fetch("https://erp-construction.onrender.com/api/project-director/ai-chat", {
        method: "POST",
        headers: { 
    ...
    <div className="flex flex-col h-[calc(100vh-180px)] max-w-4xl mx-auto bg-[#111C30] border border-slate-800 rounded-xl overflow-hidden">
      ...
      <div className="bg-[#0f182a] p-4 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="font-bold text-white text-sm">AI Project Assistant (PD)</div>
